...
from model import BertForSequenceClassification, BertForSequenceClassificationVideo, BertConfig, WEIGHTS_NAME, CONFIG_NAME
...
def eval_sims_regression(test_preds, test_truth):
...
def main(i):
    ...
    model = BertForSequenceClassificationVideo.from_pretrained(args.bert_model, cache_dir=cache_dir, num_labels = num_labels)
    ...
    nb_tr_steps = 0
    tr_loss = 0
    audio = pickle.load(open('data/audio/SIMS_audio_CLS.pickle','rb'))
    train_audio,valid_audio,test_audio = audio['train']['audio'],audio['valid']['audio'],audio['test']['audio'] 

    ## ADDITIONAL ##
    video = pickle.load(open('data/video/SIMS_video_CLS.pickle','rb'))
    train_video,valid_video,test_video = video['train']['vision'], video['valid']['vision'], video['test']['vision']
    ...
    if args.do_train:
        ...
        ## MODIFIED ##
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_train_audio, all_train_video, all_label_ids)
        ...
        ## MODIFIED ##
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,all_valid_audio, all_valid_video, all_label_ids)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                ## MODIFIED ##
                input_ids, input_mask, segment_ids, all_train_audio, all_train_video, label_ids = batch
                loss = model(input_ids, all_train_audio, all_train_video, segment_ids, input_mask, label_ids)
            ...
            ## MODIFIED ##
            for input_ids, input_mask, segment_ids,all_valid_audio, all_valid_video, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                ## ADDITIONAL ##
                all_valid_video = all_valid_video.to(device)

                ## MODIFIED ##
                with torch.no_grad():
                    tmp_eval_loss = model(input_ids, all_valid_audio, all_valid_video, segment_ids, input_mask,label_ids)
                    logits,_,_ = model(input_ids,all_valid_audio, all_valid_video, segment_ids, input_mask)
    ...
    if args.do_test:
        ...
        ## MODIFIED ##
        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_test_audio, all_test_video)
        ...
        with torch.no_grad():
            ## MODIFIED ##
            for input_ids, input_mask, segment_ids, label_ids, all_test_audio, all_test_video in tqdm(test_dataloader, desc="Evaluating"):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                label_ids = label_ids.to(device)
                ...
                ## ADDITIONAL ##
                all_test_video = all_test_video.to(device)

                with torch.no_grad():
                    ## MODIFIED ##
                    tmp_test_loss = model(input_ids, all_test_audio, all_test_video, segment_ids, input_mask, label_ids)
                    logits,text_attention,fusion_attention = model(input_ids, all_test_audio, all_test_video, segment_ids, input_mask)
        ...
        results = eval_sims_regression(predict_list, truth_list)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        results = main(i)
        acc2_list.append(str(results['Mult_acc_2']))
        F1_list.append(str(results['F1_score']))
        mae_list.append(str(results['MAE']))
        corr_list.append(str(results['Corr']))
        acc3_list.append(str(results['Mult_acc_3']))
        acc5_list.append(str(results['Mult_acc_5']))
        os.system('rm -r CM-BERT_output')

    acc2_array = np.array(acc2_list).astype(float)
    F1_array = np.array(F1_list).astype(float)
    mae_array = np.array(mae_list).astype(float)
    corr_array = np.array(corr_list).astype(float)
    acc3_array = np.array(acc3_list).astype(float)
    acc5_array = np.array(acc5_list).astype(float)
    acc2_list.append(np.mean(acc2_array))
    F1_list.append(np.mean(F1_array))
    mae_list.append(np.mean(mae_array))
    corr_list.append(np.mean(corr_array))
    acc3_list.append(np.mean(acc3_array))
    acc5_list.append(np.mean(acc5_array))
    print('acc2:',acc2_list)
    print('F1:',F1_list)
    print('mae:',mae_list)
    print('corr:',corr_list)
    print('acc3:',acc3_list)
    print('acc5:',acc5_list)
