...
class BertFinetun(nn.Module):
    def __init__(self, config):
        ...
        self.proj_a = nn.Conv1d(33,30, kernel_size=1, padding=0, bias=False)
...
## ADDITIONAL ##
class BertFinetunVideo(nn.Module):
    def __init__(self, config):
        super(BertFinetunVideo, self).__init__()
        self.proj_t = nn.Conv1d(768,30, kernel_size=1, padding=0, bias=False)
        self.proj_a = nn.Conv1d(33,30, kernel_size=1, padding=0, bias=False)
        self.proj_v = nn.Conv1d(709,30, kernel_size=1, padding=0, bias=False)
        self.activation = nn.ReLU()
        self.audio_weight_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.text_weight_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
    ...
    def forward(self, hidden_states,pooled_output,audio_data,video_data,attention_mask):
        ...
        video_data = self.proj_v(video_data)
        video_data = video_data.transpose(1, 2)
        ...
        # MODIFY
        audio_att = audio_att[:, :50, :50]
        video_att = video_att[:, :50, :50]
        
        fusion_att =text_weight_1*text_att1 + audio_weight_1*audio_att + video_weight_1*video_att + bias 
        fusion_att1 = self.activation(fusion_att)
        fusion_att = fusion_att+ attention_mask+ attention_mask_
        fusion_att = nn.Softmax(dim=-1)(fusion_att)
        fusion_att = self.dropout1(fusion_att)
...
## ADDITIONAL ##
class BertFinetunVideoNormalized(nn.Module):
    def __init__(self, config):
        super(BertFinetunVideoNormalized, self).__init__()
        self.proj_t = nn.Conv1d(768,30, kernel_size=1, padding=0, bias=False)
    ...
    def forward(self, hidden_states,pooled_output,audio_data,video_data,attention_mask):
        ...
        audio_data = self.proj_a(audio_data)
        audio_data = audio_data.transpose(1, 2)
        audio_data_1 = audio_data.reshape(-1).cpu().detach().numpy()
        weights = np.sqrt(np.linalg.norm(audio_data_1,ord=2))
        audio_data = audio_data/weights
        
        video_data = video_data.transpose(1, 2)
        video_data = self.proj_v(video_data)
        video_data = video_data.transpose(1, 2)
        video_data_1 = video_data.reshape(-1).cpu().detach().numpy()
        weights = np.sqrt(np.linalg.norm(video_data_1,ord=2))
        video_data = video_data/weights
...
## ADDITIONAL ##
class BertForSequenceClassificationVideo(BertPreTrainedModel):
    """BERT model for classification with additional video modality.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    """
    def __init__(self, config, num_labels):
        super(BertForSequenceClassificationVideo, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.BertFinetun = BertFinetunVideo(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)#拿出来CLS的表示dimension = 768
        self.classifier = nn.Linear(config.hidden_size, 1)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, all_audio_data, all_video_data, token_type_ids=None, attention_mask=None, labels=None):
        encoder_lastoutput, pooled_output, extend_mask = self.bert(input_ids,all_audio_data,token_type_ids, attention_mask, output_all_encoded_layers=True)
        pooled_output = self.dropout(pooled_output)
        pooled_output,text_att,fusion_att = self.BertFinetun(encoder_lastoutput,pooled_output,all_audio_data,all_video_data, extend_mask)
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        if labels is not None:
            loss = 0.5 * (logits.view(-1) - labels) ** 2 
...
## ADDITIONAL ##
class BertForSequenceClassificationVideoNormalized(BertPreTrainedModel):
    """BERT model for classification with additional video modality (normalized).
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    """
    def __init__(self, config, num_labels):
        super(BertForSequenceClassificationVideoNormalized, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.BertFinetun = BertFinetunVideoNormalized(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)#拿出来CLS的表示dimension = 768
        self.classifier = nn.Linear(config.hidden_size, 1)
        self.apply(self.init_bert_weights)
